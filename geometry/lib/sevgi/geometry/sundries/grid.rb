  ...
  module Geometry
    class Grid < Tile
      ...
      def initialize(x:, y:)
        ...
        super(Rect[@x.u, @y.u], nx: @x.n, ny: @y.n)
      end

      def canvas = Graphics::Canvas.new(**Graphics::Size[x.brut, y.brut].to_h, margins: Graphics::Margin[y.margin, x.margin].to_a)
      ...
      class X < Axis
        def line = @line ||= Line[d, 0.0]
      end

      class Y < Axis
        def initialize(this, other) = super(other, this)

        def line                    = @line || Line[d, 90.0]
      end
