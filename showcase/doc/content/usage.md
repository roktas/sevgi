...
`Load "palette"` evaluates another `.sevgi` source during the current script execution (`palette` is only an example).
This call loads `palette.sevgi` from the directory that contains the active source. `Load` does not resolve the file from
the process working directory. A drawing split across several files can then move as one directory. Repeated
non-recursive loads run again. Loading a source that is already active in the same chain raises a captured cycle error.
