...
They cover executable `.sevgi` files, library use, and the public component APIs. Internal helpers can change without
notice.
...
Prefer documented DSL words in scripts: `SVG`, SVG element names, and helpers such as `TileX`. In library code, use
capitalized SVG-domain operations through the `SVG` facade, such as `SVG.Canvas`. Use double colons for types and
namespaces, such as `SVG::Canvas`. Use regular Ruby for loops, data structures, calculations, and small helper objects.
...
Advanced extensions can rely on the documented `Sevgi::F` facade and the public values it returns. The supported
...
If the docs and examples do not cover a behavior, first check whether a direct SVG element can do the job. Otherwise,
add a documented helper and a focused test before scripts depend on it.
